...
replacement_dict = {'e': 'е', 'y': 'у', 'u': 'и', 'o': 'о', 'p': 'р', 'a': 'а', 'k': 'к', 'x': 'х', 'c': 'с',
                    ...
                    'Ꮧ': 'л', 'Ꮯ': 'с', 'ᴄ': 'с', 'ɜ': 'з', 'ɯ': 'ш', 'ʙ': 'в', 'ᴧ': 'л', 'ᴨ': 'п', 'ᴩ': 'р', 'ᴋ': 'к', 'ᴀ': 'а', 'ʍ': 'м', 'ɸ': 'ф',
                    'α': 'а', 'ρ': 'р', 'z': 'з', 'u': 'и', 'Y': 'у'}
translation_table = str.maketrans(replacement_dict)

# Создаем список исключений
chenal = ['geoscan', 'findprjbot', 'teoboxbot']
exclusions = ['geoscan', 'findprjbot', 'teoboxbot', 'Vedeneev', 'Lavrinovich_A', 'AleksSp']
...
TOKEN, CHAT_IDS, OWNER_ID, TRUE_GUESTS_ID = read_token_and_chat_id()
...
def handle_photo(message):
    ...
    if str(user_id) in admin_values or str(user_id) in {OWNER_ID, ANONYMOUS_ADMIN_ID, SERVICE_CHAT_ID, TRUE_GUESTS_ID}:
        return
    ...
    if message.photo:
        ...
        with requests.get(file_url) as response:
            ...
            extracted_text = extracted_text.replace('@', 'а')
